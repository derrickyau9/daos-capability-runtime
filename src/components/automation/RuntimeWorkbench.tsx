...
export function RuntimeWorkbench() {
  ...
  return <section className="runtime-workbench">
    <div className="runtime-heading"><div><p className="eyebrow">01 / Control room</p><h2>From intent to repeatable work</h2></div><span className="training-label">SYNTHETIC DATA · LOCAL SESSION</span></div>
    <div className="runtime-columns"><GlassPanel className="runtime-form" padding="lg">
      ...
      <label>Goal<textarea value={task} onChange={event => setTask(event.target.value)} disabled={mode==='replay'} maxLength={1000}/></label>
      ...
      <label>Runtime condition<select value={scenario} onChange={event => setScenario(event.target.value)}><option value="normal">Normal operation</option><option value="session-expired">Expired session → operator handoff</option><option value="interstitial">Known maintenance notice</option><option value="slow">Slow record load</option><option value="permission">Permission denied</option><option value="unknown-dialog">Unexpected dialog</option><option value="timeout">Load timeout</option><option value="drift">Changed control label</option></select></label>
      {mode==='discovery' && <label>Model connection<select value={provider} onChange={event => setProvider(event.target.value)}><option value="codex">Codex · existing login</option><option value="openai">OpenAI · API key</option></select></label>}
      <div className="runtime-submit"><GlassButton variant="primary" icon={mode==='discovery'?<Sparkles size={16}/>:<Play size={16}/>} onClick={() => void start()} disabled={active} loading={busy}>{mode==='discovery'?'Discover workflow':'Run capability'}</GlassButton><span>{mode==='replay'?'Saved artifact · zero model calls':'Live observations · bounded decisions'}</span></div>
      {message && <p className="runtime-error" role="alert">{message}</p>}
    </GlassPanel><GlassPanel className="runtime-sessions" padding="lg"><div className="panel-heading"><h3>Session activity</h3><span>{runs.length} runs</span></div>
      {!runs.length && <div className="runtime-empty"><div className="runtime-orbit"><Play size={22}/></div><h3>Ready for the first run</h3><p>Start a capability to follow its checkpoints, ownership and outcome here.</p><span>Search → Member → Setup → Review</span></div>}
      {runs.slice(0,4).map(run => <article className={`runtime-run ${run.status==='blocked'?'needs-human':''}`} key={run.id}><div className="runtime-run-title"><strong>{run.mode==='discovery'?'Workflow discovery':'Savings review'}</strong><span className={`run-badge status-${run.status}`}>{run.result?.status || run.status}</span></div><p>{run.intervention?.reason || run.result?.code || `At step ${run.step + 1}`}</p><div className="runtime-run-meta"><span>Owner: {run.control.owner}</span><span>{run.llmCalls} model calls</span><span>{run.recoveries} recoveries</span></div><code>{run.sessionId.slice(0,18)}…</code><div className="runtime-run-actions"><a href={`/operator/${run.id}`} target="_blank" rel="noreferrer"><UserRound size={14}/>{run.status==='blocked'?'Take over session':'View session'}<ArrowUpRight size={13}/></a>{run.status!=='done' && <button onClick={() => void stop(run.id)}><CircleStop size={14}/>Stop</button>}{run.result?.status==='success' && run.mode==='discovery' && <a href={`/api/automation/runs/${run.id}/capability`} target="_blank" rel="noreferrer">Capability JSON<ArrowUpRight size={13}/></a>}</div></article>)}
    </GlassPanel></div>
  </section>;
}
